...
Agents may create planning artifacts in ad-hoc locations (`docs/superpowers/`, `.cursor/plans/`, `.claude/drafts/`, etc.). Run `pnpm --package=@finografic/project-scripts dlx triage-docs` to scan these locations and interactively move files to the correct destination (see `.agents/skills/triage-docs/SKILL.md`). If that is unavailable, manually classify each file as a spec (`docs/specs/`) or scratch (`docs/drafts/`) using the criteria above.
