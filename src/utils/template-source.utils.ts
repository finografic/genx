...
/**
 * Files stored in `_templates/` with a `.template` suffix so they are not mistaken for live agent docs at
 * repo root. When copying or reading for output, use the key (target basename).
 */
...
/**
 * Resolve the path to a bundled template file under `templateDir` for a given **output** path (what appears
 * in created/migrated packages). Maps `AGENTS.md` → `AGENTS.md.template`, `CLAUDE.md` → `CLAUDE.md.template`;
