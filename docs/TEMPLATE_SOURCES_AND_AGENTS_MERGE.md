...
Canonical shared content lives under **`_templates/`** (e.g. `_templates/AGENTS.md`, `_templates/.github/instructions/…`). Features must load template paths via the templates directory (e.g. `getTemplatesDir()`), not by reading “whatever is at repo root.”

## Problem we hit

When syncing **`AGENTS.md`** with a **reverse apply** (target keeps repo-specific sections; shared lists come from the template), a simple **walk of the target file** plus **append missing template sections** produced **`## Rules — Project-Specific` at the bottom** whenever that block was missing from the target walk or only supplied from the template append step. That contradicted **`_templates/AGENTS.md`**, where **Project-Specific is first**, then **General**, then **Markdown Tables**, then **Git Policy**.
...
3. **Reorder** the merged section list to match **`_templates/AGENTS.md`**:
...
- Extras such as **Skills** appear **after** the spine (matching **`_templates/AGENTS.md`**, which has no Skills section).
