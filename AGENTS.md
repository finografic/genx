...
- `genx create` treats `oxc-config` and `markdown` as fixed baseline features: they are always included in the returned create config and are excluded from the optional feature picker; package-type `defaultFeatures` should only represent user-toggleable extras on top of that baseline
...
- Multi-repo execution uses `genx managed <command>` (subcommands: `upgrade`, `deps`, `audit`); the shared target loop lives in `src/lib/managed/managed-loop.runner.ts` (`runManagedLoop`); `--managed` flag is kept as a deprecated compatibility alias with a warning
